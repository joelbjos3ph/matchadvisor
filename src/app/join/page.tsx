...
export default function JoinPage() {
  ...
  const [licenceFileError, setLicenceFileError] = useState("");
  ...
  function validate(): boolean {
    ...
    let fileValid = true;
    if (!licenceFile) {
      setLicenceFileError("Please upload your licence card or certificate.");
      fileValid = false;
    } else {
      setLicenceFileError("");
    }
    if (!selfieFile) {
      setSelfieFileError("Please upload a selfie holding your licence.");
      fileValid = false;
    } else {
      setSelfieFileError("");
    }

    return Object.keys(next).length === 0 && fileValid;
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const licenceExt = licenceFile!.name.split(".").pop();
    const licencePath = `professionals/${Date.now()}-${Math.random().toString(36).slice(2)}-licence.${licenceExt}`;
    const { error: licenceUploadError } = await supabase.storage
      .from("licence-documents")
      .upload(licencePath, licenceFile!, { contentType: licenceFile!.type });

    if (licenceUploadError) {
      console.error("[Storage error — licence upload]", licenceUploadError);
      setSubmitError(`Failed to upload licence document: ${licenceUploadError.message}`);
      setLoading(false);
      return;
    }
    ...
    const selfiePath = `professionals/${Date.now()}-${Math.random().toString(36).slice(2)}-selfie.${selfieExt}`;
    const { error: selfieUploadError } = await supabase.storage
      .from("licence-documents")
      .upload(selfiePath, selfieFile!, { contentType: selfieFile!.type });

    if (selfieUploadError) {
      console.error("[Storage error — selfie upload]", selfieUploadError);
      setSubmitError(`Failed to upload selfie: ${selfieUploadError.message}`);
      setLoading(false);
      return;
    }
    ...
    const { error } = await supabase.from("professionals").insert({
      ...
      bio: form.bio,
      verified: false,
      licence_document_url: licencePath,
        ...
        <section className="flex items-center justify-center min-h-[calc(100vh-64px)] px-6">
          <div className="max-w-md w-full text-center">
            ...
            <p className="text-slate-500 leading-relaxed mb-8">
              ...
              application and will review your credentials within 2–3 business days. We&apos;ll be in touch at{" "}
              <span className="font-medium text-slate-700">{form.email}</span>.
            </p>
            <a href="/" className="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-6 py-3 rounded-xl transition-colors">
              Back to home
            </a>
          </div>
        </section>
    ...
    <main className="min-h-screen bg-white">
      ...
      <section className="py-12 px-6 pb-20">
        <div className="max-w-2xl mx-auto">
          <form onSubmit={handleSubmit} noValidate className="bg-white border border-slate-200 rounded-2xl p-8 sm:p-10 shadow-sm">

            <h2 className="text-lg font-semibold text-slate-900 mb-6">Personal details</h2>

            <div className="grid sm:grid-cols-2 gap-5 mb-5">
              <Field label="Full name" error={errors.fullName} required>
                <input type="text" name="fullName" value={form.fullName} onChange={handleChange} placeholder="Jane Tan" className={inputCls(!!errors.fullName)} />
              </Field>
              <Field label="Email address" error={errors.email} required>
                <input type="email" name="email" value={form.email} onChange={handleChange} placeholder="jane@example.com" className={inputCls(!!errors.email)} />
              </Field>
            </div>

            <div className="mb-5">
              <Field label="Phone number" error={errors.phone} required>
                <input type="tel" name="phone" value={form.phone} onChange={handleChange} placeholder="+65 9123 4567" className={inputCls(!!errors.phone)} />
              </Field>
            </div>
            ...
            <div className="border-t border-slate-100 pt-8 mt-8">
              ...
              <div className="mb-5">
                <Field label="Profession type" error={errors.profession} required>
                  <select name="profession" value={form.profession} onChange={handleChange} className={inputCls(!!errors.profession)}>
              ...
              <div className="grid sm:grid-cols-2 gap-5 mb-5">
                <Field label="Licence / registration number" error={errors.licenceNumber} required hint="e.g. MAS FA number, CEA registration">
                  <input type="text" name="licenceNumber" value={form.licenceNumber} onChange={handleChange} placeholder="e.g. FA12345" className={inputCls(!!errors.licenceNumber)} />
                </Field>
                <Field label="Years of experience" error={errors.yearsOfExperience} required>
                  <input type="number" name="yearsOfExperience" value={form.yearsOfExperience} onChange={handleChange} min={0} max={60} placeholder="e.g. 5" className={inputCls(!!errors.yearsOfExperience)} />
                </Field>
              </div>
              ...
              <div className="mb-5">
                <FileUploadZone
                  label="Upload your licence card or certificate"
                  accept=".pdf,.jpg,.jpeg,.png"
                  acceptLabel="PDF, JPG or PNG"
                  maxSizeMB={MAX_FILE_SIZE_MB}
                  file={licenceFile}
                  onChange={(f) => { setLicenceFile(f); if (f) setLicenceFileError(""); }}
                  error={licenceFileError}
                  required
                />
              </div>
              ...
              <div className="mb-5">
                <FileUploadZone
                  label="Upload a selfie holding your licence"
                  accept=".jpg,.jpeg,.png"
                  acceptLabel="JPG or PNG"
                  maxSizeMB={MAX_FILE_SIZE_MB}
                  file={selfieFile}
                  onChange={(f) => { setSelfieFile(f); if (f) setSelfieFileError(""); }}
                  error={selfieFileError}
                  required
                />
              </div>

              <div className="mb-8">
                <Field label="Short bio" error={errors.bio} required hint={`${form.bio.length} characters — minimum 30`}>
                  <textarea name="bio" value={form.bio} onChange={handleChange} rows={5} placeholder="Tell potential clients about your background, specialisations, and what makes you the right professional for them..." className={`${inputCls(!!errors.bio)} resize-none`} />
                </Field>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 mb-8 text-sm text-slate-500">
              ...
              <span>
                ...
                We verify all licences and credentials before approving your listing.
              </span>
            </div>
    ...
    <div>
      ...
      <div
        ...
        className={[
          ...
          file && !hasError ? "border-green-300 bg-green-50" : "",
          hasError ? "border-red-300 bg-red-50" : "",
          !isDragging && !file && !hasError ? "border-slate-200 bg-slate-50 hover:border-indigo-300 hover:bg-indigo-50" : "",
        ].join(" ")}
      >
        <input
          ref={inputRef}
        ...
        {file && !hasError ? (
          ...
          <>
            ...
            <p className="text-xs text-slate-400">{acceptLabel} · Max {maxSizeMB}MB</p>
          </>
        )}
      </div>
